...
from excel_to_postgres.json_builder import JSONBuilder
...
def run_migration(config: Config):
    ...
    print(f"Target Database Table: {target_table}")
    ...
    builder = JSONBuilder(semester=config.semester)
    ...
    try:
        for sheet_name, row_idx, row_dict, row_list in reader.read_workbook():
            total_rows += 1
            try:
                student_id, name, email, data_json = builder.process_row(sheet_name, row_dict, row_list)
        ...
        if db and not config.dry_run:
            db.commit()
            logger.info("Committed transaction to database '%s' table '%s' successfully.", config.db_name, target_table)
