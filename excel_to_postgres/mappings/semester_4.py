...
SUBJECT_KEYS_SEM4 = [
    ...
    ("collections and stream api", "collections_stream_api"),
    ("collections", "collections_stream_api"),
    ("java advanced", "java_advanced"),
]

ASSESSMENT_KEYS_SEM4 = [
...
def create_empty_sem4_schema(sheet_name: str) -> dict:
    subjects = {}
    track_results = {}

    unique_keys = sorted(list(set(k for _, k in SUBJECT_KEYS_SEM4)))
    for subj_key in unique_keys:
        subjects[subj_key] = {
            "levels": None,
            "inputs": {
...
class Semester4Mapping(SemesterMapping):
    ...
    def _match_subject_key(self, text: str) -> str | None:
        ...
        for keyword, key in SUBJECT_KEYS_SEM4:
    ...
    def _match_assessment(self, top_n: str, bot_n: str) -> tuple[str | None, str | None]:
        combined = f"{top_n} {bot_n}".strip()
        for keyword, asm_key in ASSESSMENT_KEYS_SEM4:
            if keyword in top_n:
                rem = bot_n
                return asm_key, rem
    ...
    def build_json(self, sheet_name: str, row_dict: dict, row_list: list) -> dict:
        data = create_empty_sem4_schema(sheet_name)
